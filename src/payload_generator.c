...
void calculate_hash(uint8_t* hash_destination, uint32_t data_size) {
	...
	if (data_size < MD5_BUFFER_SIZE_BYTES)
		...
		for (size = 0; data_size >= MD5_BUFFER_SIZE_BYTES; data_size -=
			...
			MD5_Update(&ctx, &hash_destination[index], MD5_BUFFER_SIZE_BYTES);
		}

		/* Calculate the hash of the remaining bytes [< MD5_BUFFER_SIZE_BYTES] */
		/* only if data size is > 0 [Avoid double hash calculation] */
		if (data_size > 0)
			MD5_Update(&ctx, &hash_destination[MD5_HASH_SIZE_BYTES + size],
					data_size);
	}
...
int write_header(void) {
	...
	/* Number of chunks */
	chunks = ((FLASH_USER_SECTORS_4K - 1) * PAYLOAD_BLOCK_PIECES)
			+ ((FLASH_USER_SECTORS_32K - 1) * PAYLOAD_BLOCK_PIECES
					* PAYLOAD_BLOCK_PIECES_32K);
	...
	iap_status = (e_iap_status) iap_copy_ram_to_flash(&block,
			(void *) sector_start_address[FLASH_USER_HEADER_SECTOR], SIZE_4096);
	if (iap_status != CMD_SUCCESS)
		return iap_status;
...
int write_payload(void) {
	...
	while (sector <= FLASH_USER_PAYLOAD_END_SECTOR) {
		...
		for (payload_piece = 0; payload_piece < PAYLOAD_BLOCK_PIECES;
				++payload_piece) {
		...
		if ((sector >= FLASH_SECTOR_16)
				&& (sub_blocks < PAYLOAD_BLOCK_PIECES_32K)) {
			++sub_blocks;
		} else {
			sub_blocks = 1;
			++sector;
...
int write_end(void) {
	...
	/* Write payload block to flash */
	iap_status = (e_iap_status) iap_copy_ram_to_flash(&block,
			(void *) sector_start_address[FLASH_USER_END_SECTOR], SIZE_4096);
	if (iap_status != CMD_SUCCESS)
		return iap_status;
